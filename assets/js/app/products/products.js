  ...
  "columns": [
    ...
    { "data": "name" },
    {
      ...
      "render": function (data, type, row) {
        return "<button class='btn btn-outline-info me-1'  data-id='" + data.product_id + "' onclick='OpenModalInfo(this)'><i class='fa-solid fa-eye'></i></button>" +
          "<button class='btn btn-outline-warning me-1' data-id='" + data.product_id + "' onclick='OpenModalEdit(this)' ><i class='fa-solid fa-pen'></i></button>" +
          "<button class='btn btn-outline-danger' data-id='" + data.product_id + "' data-name='" + data.name + "' onclick='OpenModalDel(this)'><i class='fa-solid fa-trash'></i></button>";
      }
    }
  ],
    ...
    },
    {
      ...
      action: function (e, dt, node, config) {
        ...
        if (selectedRows.length > 0) {
          updateSelectedProductsList(selectedRows); 
...
function updateSelectedProductsList() {
  var selectedRows = table.rows({ selected: true }).data();
  var listContainer = $('#selectedProductsList');
  listContainer.empty(); 
  selectedRows.each(function(rowData) {
      var productName = rowData.name; 
      listContainer.append('<li>' + productName + '</li>'); 
  });
}
...
function OpenModalInfo(data) {
  var product_id = data.getAttribute('data-id')
  fetch('/products_api/products/' + product_id, {
    method: 'GET',
    headers: {
      'Content-Type': 'application/json'
    }
    ...
    .then(productData => {
      $('#modalCoverInfo').find('#infoId').val(productData.product_id);
      $('#modalCoverInfo').find('#infoName').val(productData.name);
...
function OpenModalEdit(data) {
  ...
  fetch('/products_api/products/' + product_id, {
    ...
    .then(productData => {
      $('#modalCoverEdit').find('#editId').val(productData.product_id);
      $('#modalCoverEdit').find('#editName').val(productData.name);
...
function OpenModalDel(data) {
  ...
  document.getElementById('modal_submit_del_product').value = product_id;
